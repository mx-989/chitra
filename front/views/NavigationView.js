...
async showTimelineSection() {
    ...
    this.initTimelineStyles(); 
    this.loadAllStats();
    this.loadTimelinePhotos();
}

initTimelineStyles() {
    if (document.querySelector('#timeline-styles')) return;
    ...
    style.textContent = `
        ...
        .date-line {
            flex: 1;
            height: 2px;
            background: linear-gradient(to right, var(--chitra-primary), transparent);
            margin-left: -1rem;
            border-radius: 1px;
        }
        .photo-info {
        ...
        .photo-time, .photo-album {
            display: flex;
            align-items: center;
            gap: 0.3rem;
        }
        .photo-time i, .photo-album i {
            font-size: 0.7rem;
            color: var(--chitra-primary);
            width: 12px;
        }
